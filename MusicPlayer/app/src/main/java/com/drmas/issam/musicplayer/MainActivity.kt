package com.drmas.issam.musicplayer
...
class MainActivity : AppCompatActivity() {
    ...
    var songArtis: TextView? = null

    private var mediaPlayyer: MediaPlayer? = null
        ...
        launch (kotlinx.coroutines.experimental.android.UI){
            ...
            val playerUI = object:AnkoComponent<MainActivity> {
                ...
                fun playRandom(){
                    ...
                    val song = songs[0]
                    mediaPlayyer?.reset()
                    mediaPlayyer = MediaPlayer.create(ctx, song.uri)
                    mediaPlayyer?.setOnCompletionListener {
                        playRandom()
                    }
                    albumArt?.imageURI = song.albumArt
                    songTitle?.text = song.title
                    songArtis?.text = song.artist
                    mediaPlayyer?.start()
                    playButton?.imageResource = R.drawable.ic_pause_black_24dp
                }
                fun playOrPause(){
                    var songPlaying:Boolean? = mediaPlayyer?.isPlaying
